...
class DelayedEventsStore(SQLBaseStore):
    ...
    def _prune_excess_finalised_delayed_events_for_user(
        ...
        if num_existing > retention_limit:
            txn.execute(
                ...
                DELETE FROM delayed_events
                WHERE delay_id IN (
                    SELECT delay_id FROM delayed_events
                    WHERE user_localpart = ?
                        AND finalised_ts IS NOT NULL
    ...
    async def get_delayed_event_for_user(
        ...
        """
        Returns the specified scheduled delayed event owned by the given user.
    ...
    async def process_target_delayed_event(
        ...
        Returns: The details of the matching delayed event,
            and the send time of the next delayed event to be sent, if any.
            Both are None if the delayed event has already been sent.
    ...
    async def cancel_delayed_event(
        ...
        """
        Cancels the matching delayed event, i.e. finalise it as long as it hasn't been processed.
    ...
    async def cancel_delayed_state_events(
        ...
        *,
        ...
        """
        Cancels all matching delayed state events, i.e. finalise them as long as they haven't been processed.
        ...
        Args:
            ...
            finalised_ts: The timestamp to record as the cancellation time.
    ...
    async def finalise_processed_delayed_event(
        ...
        Args:
            delay_id: The ID of the delayed event to finalise.
            result_or_error: The ID of the event that was sent, or the standard error
                response of the error that prevented it from being sent.
            finalised_ts: The timestamp to record as the finalisation time.
    ...
    def _get_next_delayed_event_send_ts_txn(
        ...
        resp = txn.fetchone()
        return Timestamp(resp[0]) if resp is not None and resp[0] is not None else None
