...
class DelayedEventsHandler:
    ...
    async def _prune_finalised_events(self) -> None:
        await self._store.prune_finalised_delayed_events(
            ...
            self._config.experimental.msc4140_finalised_retention_period,
            self._config.experimental.msc4140_finalised_per_user_retention_limit,
        )
    ...
    async def _send_event(
        ...
        """
        Sends the given delayed event, and finalises it in the DB.

        Args:
            event: The delayed event to send.
            finalise_error: Whether to finalise the delayed event with the error
                if sending it fails. If False, the error is re-raised instead.
